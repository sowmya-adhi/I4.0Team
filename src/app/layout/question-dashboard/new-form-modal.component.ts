...
export class NewFormModalComponent implements OnInit {
    ...
    submitForm() {
        ...
        options.push({
            Option: this.myForm.get('option1').value,
            //    Question_Id: this.new_question_id,
            Maturity_Level: 1
        });
        options.push({
            Option: this.myForm.get('option2').value,
            //      Question_Id: this.new_question_id,
            Maturity_Level: 2
        });
        options.push({
            Option: this.myForm.get('option3').value,
            //      Question_Id: this.new_question_id,
            Maturity_Level: 3
        });
        options.push({
            Option: this.myForm.get('option4').value,
            //     Question_Id: this.new_question_id,
            Maturity_Level: 4
        });
        options.push({
            Option: this.myForm.get('option5').value,
            //     Question_Id: this.new_question_id,
            Maturity_Level: 5
        });
        // console.log(options);
        ...
        if (this.addArea && q_t_id === 1) {
            this.HTTPSERVICE.post(this.CONSTANT.AssessmentArea_API, {
                Assessment_Area: this.myForm.get('area').value
            }).subscribe((data: any) => {
                let newarea_id = data['Assessment_Area_Id'];
                console.log('Assessment Area added with id:' + newarea_id);
                this.HTTPSERVICE.post(this.CONSTANT.Category_API, {
                    Category: this.myForm.get('category').value,
                    Assessment_Area_Id: newarea_id
                }).subscribe((data: any) => {
                    let newcategory_id = data['Category_Id'];
                    console.log('Category inserted with ID ' + newcategory_id);
                    let body = {
                    ...
                    this.HTTPSERVICE.post(this.CONSTANT.Question_API, body).subscribe(async (data: any) => {
                        console.log('question instered with id:' + data['Question_Id']);
                        ...
                        for (let i = 0; i < options.length; i++) {
                            let body: {
                                ...
                                Maturity_Level: options[i]['Maturity_Level']
                            };
                            await this.delay(100);
                            this.HTTPSERVICE.post(this.CONSTANT.Question_Option_Api, body).subscribe((data: any) => {
        ...
        } else if (this.addCategory && q_t_id === 1) {
            let area_id: number;
            for (let i = 0; i < this.area_arr.length; i++) {
                if (this.area_arr[i]['Assessment_Area'] === this.selected_area)
                    area_id = Number(this.area_arr[i]['Assessment_Area_Id']);
            ...
            getSubscribe.subscribe(async (data: any) => {
        ...
        if (this.addArea && q_t_id === 2) {
            this.HTTPSERVICE.post(this.CONSTANT.AssessmentArea_API, {
                ...
                this.HTTPSERVICE.post(this.CONSTANT.Category_API, {
                    ...
                    this.HTTPSERVICE.post(this.CONSTANT.Question_API, body).subscribe((data: any) => {
                        console.log('question instered with id:' + data['Question_Id']);
                        this.activeModal.dismiss('Done');
                    });
                });
            });
        } else if (this.addCategory && q_t_id === 2) {
            let area_id: number;
            for (let i = 0; i < this.area_arr.length; i++) {
                if (this.area_arr[i]['Assessment_Area'] === this.selected_area)
                    area_id = Number(this.area_arr[i]['Assessment_Area_Id']);
